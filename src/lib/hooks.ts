...
export function useTrackList(tracks: Track[]) {
    ...
    const [cats, setCats] = useState<CategoryFilters>({ music: true, sessions: true, productions: true });
    const [years, setYears] = useState<number[]>([]);
    const [keys, setKeys] = useState<string[]>([]);
    const [bpmRange, setBpmRange] = useState<[number, number]>([BPM_MIN, BPM_MAX]);
    const [favs, setFavs] = useState<FavoriteFilters>({ star: false, heart: false });
    const [sort, setSort] = useState<SortConfig>({ field: 'id', dir: 'asc' });
    ...
    const filtered = useMemo(() => {
        ...
        if (years.length) r = r.filter(t => years.includes(t.year));
        if (keys.length) r = r.filter(t => t.key.some(k => keys.includes(k)));
        if (favs.star && favs.heart) r = r.filter(t => t.star || t.heart);
        else if (favs.star) r = r.filter(t => t.star);
        else if (favs.heart) r = r.filter(t => t.heart);
        r = r.filter((t) => {
            const n = bpmNum(t.bpm);
            if (n == null) return bpmRange[0] === BPM_MIN && bpmRange[1] === BPM_MAX;
        ...
        return r;
    }, [q, cats, years, keys, bpmRange, favs, sort, tracks]);

    useEffect(() => setPage(0), [q, cats, years, keys, bpmRange, favs, sort]);
